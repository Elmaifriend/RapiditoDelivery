<div class="flex flex-col min-h-screen bg-gray-50/50 pb-20">
    ...
    {{-- CABECERA DE ÉXITO --}}
    <div class="bg-white px-4 pt-12 pb-8 border-b border-gray-100 text-center space-y-4">
        {{-- Icono animado de Checkmark --}}
        <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-green-50 text-green-500 text-2xl animate-bounce">
    ...
    <div class="max-w-md mx-auto w-full space-y-3 p-4">
        ...
        <div class="bg-white rounded-2xl p-4 border border-gray-100/80 space-y-2">
            ...
            <div class="space-y-1.5">
                ...
                @if($dropoff?->reference)
                    <div class="bg-amber-50/70 border border-amber-100/50 rounded-xl p-2.5">
                        <p class="text-[10px] font-black uppercase tracking-wider text-amber-800">Referencia aportada:</p>
                        <p class="text-xs text-amber-900 font-medium mt-0.5">{{ $dropoff->reference }}</p>
                    </div>
                @endif

                @if($order->special_instructions)
                    <div class="bg-gray-50 border border-gray-100 rounded-xl p-2.5">
                        <p class="text-[10px] font-black uppercase tracking-wider text-gray-500">Instrucciones especiales:</p>
                        <p class="text-xs text-gray-700 font-medium mt-0.5">{{ $order->special_instructions }}</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-4 border border-gray-100/80 flex items-center justify-between">
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    <p class="text-[10px] font-black uppercase tracking-wider text-gray-400">Método de pago</p>
        ...
        <div class="bg-white rounded-2xl border border-gray-100/80 overflow-hidden">
                    ...
                    <div class="p-4 flex items-center justify-between gap-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="space-y-0.5">
                                <p class="text-sm font-bold text-gray-900">{{ $item->name ?? $item->menuItem?->name }}</p>
                                @if($item->notes)
            ...
            <div class="p-4 bg-gray-50/50 border-t border-gray-100 space-y-2">
                ...
                <div class="flex justify-between text-xs font-semibold text-gray-500">
                    ...
                    <span class="font-mono">${{ number_format($order->delivery_fee, 2) }}</span>
                </div>

                <div class="flex justify-between text-base font-black text-gray-900 pt-2 border-t border-gray-200/60">
                    <span>Total de la Orden</span>
                    <span class="font-mono text-lg text-red-500">${{ number_format($order->total, 2) }}</span>
                </div>
            </div>
        </div>
