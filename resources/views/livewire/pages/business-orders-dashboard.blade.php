            ...
            @if ($activeTab === 'nuevos')
                ...
                @if ($pedidosNuevos->isEmpty())
                    <div
                        ...
                        <p class="mt-2 max-w-[250px] text-sm text-gray-500">No hay pedidos pendientes en este momento. Te
                            avisaremos cuando llegue uno nuevo.</p>
                    </div>
                @else
                    <div class="space-y-6">
                                ...
                                <div class="mb-4 flex items-start justify-between border-b border-gray-100 pb-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="mt-0.5 flex items-center gap-1 text-xs font-semibold text-gray-500">
            ...
            @else
                ...
                @if ($pedidosAceptados->isEmpty())
                    <div
                        ...
                        <p class="mt-2 max-w-[250px] text-sm text-gray-500">No hay pedidos preparándose en este momento.</p>
                    </div>
                @else
                    <div class="space-y-6">
                        @foreach ($pedidosAceptados as $pedido)
                            ...
                            <div>
                                ...
                                @elseif ($pedido->delivery_status->value === \App\Enums\DeliveryStatus::WAITING_DRIVER->value)
                                    <div class="flex items-center justify-center gap-2 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-4 text-center text-sm font-bold text-amber-700">
                                        <span class="h-2 w-2 animate-pulse rounded-full bg-amber-500"></span>
                                        Buscando repartidor...
                                    </div>
                                @elseif ($pedido->delivery_status->value === \App\Enums\DeliveryStatus::DRIVER_HEADING_TO_RESTAURANT->value)
                                    <div class="flex items-center justify-center gap-2 rounded-2xl border border-indigo-200 bg-indigo-50 px-3 py-4 text-center text-sm font-bold text-indigo-700">
                                        <span class="h-2 w-2 animate-pulse rounded-full bg-indigo-500"></span>
                                        Repartidor en camino al local
                                    </div>
                                @elseif (in_array($pedido->delivery_status->value, [\App\Enums\DeliveryStatus::PICKED_UP->value, \App\Enums\DeliveryStatus::ON_THE_WAY->value]))
                                    <div class="flex items-center justify-center gap-2 rounded-2xl border border-sky-200 bg-sky-50 px-3 py-4 text-center text-sm font-bold text-sky-700">
